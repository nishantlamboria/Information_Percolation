...
mpl.rcParams['text.usetex'] = True  # Use mathtext, not full LaTeX

# Define points and labels
x_start, x_end = 0, 1
internal_points = [0.175, 0.55, 0.825]
labels = [
    r'$\frac{\theta}{2}$',
    r'$p(\sigma,v)$',
    r'$1-\frac{\theta}{2}$'
...
# Vivid blue segment
ax.hlines(y=y, xmin=internal_points[0], xmax=x_middle, color=soft_blue, linewidth=6)
# Vivid red segment
ax.hlines(y=y, xmin=x_middle, xmax=internal_points[2], color=soft_red, linewidth=6)
# Right soft red
ax.hlines(y=y, xmin=internal_points[2], xmax=x_end, color=vivid_red, linewidth=6)
...
plt.tight_layout()
plt.savefig("01_plot_pre.pdf", format="pdf", bbox_inches="tight")
plt.show()